...
## Phase 4 — Firebase → Supabase 이전 (진행 중)

기록은 `supabase/README.md` 에 있다. 여기서는 상태만 요약한다.

| | 상태 |
|---|---|
| 스키마 + RLS + 문서 저장 API + Realtime | 완료 (Supabase 프로젝트 `gyeol`, 서울) |
| 클라이언트 쓰기 (`saveDoc`/`removeDoc`) | 완료 — 호출부 90곳 |
| 클라이언트 읽기 (Realtime 구독) | 완료 |
| 인증 (전화번호 OTP) | 완료 — SMS 공급자 연결만 남음 |
| 서버 라우트 28곳 | **어댑터만 준비.** 배선 남음 |
...
- **권한 상승.** 본인이 자기 등급·역할·소속을 바꾸는 것을 트리거가 막는다.

### 남은 결정

1. **SMS 공급자** — OTP 발송에 필요하다. `User` 타입에 `aligoKey`·`aligoSender` 가
   이미 있어 알리고를 쓰려던 것으로 보인다. 맞다면 Supabase Send SMS Hook 으로 연결한다.
2. **소셜 로그인(카카오·네이버)** — 지금은 Supabase 세션을 만들지 않아 거부된다.
   조용히 통과시키면 그게 정확히 옛 구멍이라 막아 두었다.
   Supabase OAuth 로 옮길지, 전화번호 OTP 로 일원화할지 정해야 한다.

### 정리 대상

`firestore.rules`·`firestore.v4.rules` 와 그 테스트는 이제 떠나는 시스템을 검증한다.
서버 이전이 끝나 Firestore 의존이 사라지면 함께 지운다. 그 전까지는 남겨 둔다 —
이전이 중단될 경우 되돌아갈 자리이고, v4 규칙에는 RLS 설계의 근거가 담겨 있다.

...
begin
  ...
  perform pg_temp.expect(n = 0, '손님은 주문 테이블을 훑지 못한다');

  -- 5) 스스로 등급을 올릴 수 없다.
  begin
    perform public.save_doc('users', c::text, jsonb_build_object('role','owner'));
    perform pg_temp.expect(false, '손님이 스스로 사장이 되지 못한다');
  exception when insufficient_privilege then
    perform pg_temp.expect(true, '손님이 스스로 사장이 되지 못한다');
  end;
  ...
  -- 5-1) 권한이 오르지 않더라도 **오른 것처럼 보이면 안 된다.**
  --      앱은 행을 {...data, id} 로 읽으므로 data.role 만 바꿔도 화면은 사장이 된다.
  begin
    perform public.save_doc('users', c::text, jsonb_build_object('role','owner'));
    perform pg_temp.expect(false, 'data.role 만 사장으로 바꾸는 것도 막힌다');
  exception when insufficient_privilege then
    perform pg_temp.expect(true, 'data.role 만 사장으로 바꾸는 것도 막힌다');
  end;

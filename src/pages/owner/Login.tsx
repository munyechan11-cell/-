...
export default function OwnerLogin() {
  ...
  const handleOAuthLogin = async (provider: 'google' | 'kakao') => {
    setIsLoading(true);
    try {
      await login('', '', 'owner', true, provider);
      navigate('/owner');
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await login(email, password, 'owner', !isLogin, undefined, restaurantName);
      navigate('/owner');
    } catch (err: any) {
        ...
        <div className="space-y-8">
           <div className="flex flex-col gap-4">
              <button 
                ...
                className="w-full py-5 bg-white rounded-2xl border-2 border-[#4285F4]/30 flex items-center justify-center gap-4 hover:shadow-[0_15px_40px_rgba(66,133,244,0.2)] hover:border-[#4285F4] transition-all group relative overflow-hidden active:scale-[0.98]"
              >
           ...
           <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {isLoading ? '인증 중...' : (isLogin ? '로그인 실행' : '신규 가맹 등록')}
              </button>
           </form>
           ...
           >
             {isLogin ? '아직 회원이 아니신가요? 가맹 등록' : '이미 가맹점이신가요? 로그인'}
           </button>
        </div>
      </div>
    </div>

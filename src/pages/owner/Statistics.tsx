  ...
  return (
    <div className="min-h-full bg-slate-50 pb-20">
      ...
      <div className="bg-white text-slate-900 p-6 pt-8 border-b border-slate-100 sticky top-0 z-20">
        ...
        <p className="text-slate-500 text-sm mt-1 font-medium">매장 현황 요약</p>
      </div>
            ...
            {last7Days.map((dateStr, i) => {
              ...
              return (
                <div key={dateStr} className="flex flex-col items-center flex-1">
                  <div className="w-full relative flex justify-center items-end h-32 bg-slate-50 rounded-t-lg">
                    <div 
                      className="w-full bg-indigo-500 rounded-t-lg transition-all duration-500"
                      style={{ height: count > 0 ? height : '4px' }}
                    ></div>
                    {count > 0 && (
                      <span className="absolute -top-6 text-xs font-bold text-slate-600">{count}</span>
                    )}
                  </div>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 text-center">
              <p className="text-slate-500 text-xs font-medium mb-1">총 발급 쿠폰</p>
              <p className="text-2xl font-bold text-slate-900">{storeCoupons.length}</p>
            </div>
            <div className="bg-indigo-50 rounded-2xl p-4 border border-indigo-100 text-center">
              <p className="text-indigo-600 text-xs font-medium mb-1">사용 완료</p>
              <p className="text-2xl font-bold text-indigo-700">{usedCouponsCount}</p>
            </div>
          </div>
          
          <div className="mt-6">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-slate-500 font-medium">쿠폰 사용률</span>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-100 flex justify-around p-4 pb-safe z-40">
        ...
        <Link to="/owner/statistics" className="flex flex-col items-center text-indigo-600">

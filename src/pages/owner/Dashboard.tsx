    ...
    <div className="min-h-full bg-transparent pb-20">
      ...
      {pendingRequests.length > 0 && (
        <div className="px-6 pt-6 animate-in fade-in slide-in-from-top-4 duration-500">
          <h2 className="text-lg font-bold text-[#D84315] mb-4 flex items-center">
            <span className="bg-[#FFF3E0] w-8 h-8 rounded-full flex items-center justify-center mr-2 animate-pulse">
          ...
          <div className="space-y-3">
            {pendingRequests.map(request => {
              ...
              return (
                <div key={request.id} className="bg-white/90 backdrop-blur-sm rounded-2xl p-4 shadow-md border-2 border-[#D84315]/30 flex flex-col gap-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-bold text-[#2D1B15] text-lg">
                        {customer?.name || '알 수 없는 고객'} 
                        {request.usedAtTable && <span className="text-sm font-bold text-[#D84315] ml-2 bg-[#FFF3E0] px-2 py-0.5 rounded-md">테이블 {request.usedAtTable}</span>}
                      </p>
                      <p className="text-[#795548] font-medium mt-1">{request.description}</p>
                    </div>
                  </div>
                  <div className="flex gap-2 mt-2">
                    <button 
                      onClick={() => rejectCouponUse(request.id)}
                      className="flex-1 py-2 bg-[#EFEBE9] text-[#5D4037] rounded-xl font-bold hover:bg-[#E7E0D7] transition-colors"
                    >
                      거절
                    </button>
                    <button 
                      onClick={() => approveCouponUse(request.id)}
                      className="flex-1 py-2 bg-[#D84315] text-white rounded-xl font-bold hover:bg-[#BF360C] transition-colors shadow-sm"
                    >
                      수락
                    </button>
                  </div>

 ...
 *   store 는 이미 firebaseStatus/firebaseError 로 실패를 알고 있었지만
 *   그 값을 화면에 쓰는 곳이 한 군데도 없어서, 장애가 조용히 묻혔다.
 *   → 실패를 눈에 보이게 만들어 오진을 막는다.
 *
 * 표시 조건: firebaseStatus === "error" (리스너가 실제로 거부·실패한 경우)
 *   "offline"(Firebase 미설정 → 로컬 전용 모드)은 의도된 동작이라 표시하지 않는다.
 */
export function DbStatusBanner() {
  const lang = useLanguage();
  const { firebaseStatus, firebaseError } = useStore();

  if (firebaseStatus !== "error") return null;
      ...
      <div className="mx-auto flex max-w-3xl items-start gap-2">
        ...
        <div className="min-w-0 text-xs leading-relaxed">
          ...
          {firebaseError && (
            ...
            <p className="mt-1 break-words font-mono text-[10px] opacity-75">{firebaseError}</p>
          )}
        </div>
      </div>
    </div>

...
- [ ] **8-1. 폰트 여러 개 (디자인 테마별 선택)** — 매장 브랜드/테마에 맞는 폰트를 여러 종 제공해 선택. ⏳ 예시 폰트 파일 업로드 대기.
- [ ] **8-2. 가게별 고객용 웹사이트 연결** — 각 가게의 **공개 브랜드 사이트**를 매장 데이터(이름·메뉴·사진·리뷰·영업시간·예약)로 자동 구성. 디자인 **퀄리티/감성 레퍼런스**: https://caffiend-web-mvp.vercel.app/ (Hero→메뉴→리뷰→방문→예약→푸터, 에디토리얼·따뜻한 톤). **베끼지 않고 디자인 감성만** 차용. 흐름: 손님이 주문 후 **알림 → "가게 사이트 접속" 버튼 → 그 매장 사이트**. 기술: 공개 라우트(`/site/:storeId`, 로그인 불필요) + 서버가 공개 데이터만 제공하는 엔드포인트(`/api/site/:storeId`). 4개국어. (큰 기능)
- [ ] **8-3. 세무 AI (절세·감세)** — 과정(매출·지출·공제 분석)을 단계로 보여주고 결과(예상 세액·절세 팁·신고 준비)까지 이어지도록. 기존 지출(receipt)·매출 데이터 활용. (큰 기능)
- [ ] **8-4. 소상공인 지원정보 추천 알림** — 정부·지자체 지원금/정책자금/바우처 등 업종·지역 맞춤 추천 + 알림. (데이터 소스: 공공 API/크롤링 검토)
- [ ] **8-5. 리뷰 작성 시 쿠폰 제공** — 손님이 리뷰를 남기면 자동 쿠폰 지급(리뷰 유도). 쿠폰 시스템 + 리뷰 작성 트리거 연결.
- [ ] **8-6. (버그) 쿠폰 보낼 때 알림 미발송** — 쿠폰 발송 시 손님에게 알림이 안 감. 알림 경로(푸시/인앱/카카오) 점검·수정.
- [ ] **8-7. 금액 쿠폰 → 계산서 즉시 할인 적용** — 금액 쿠폰 사용 시 계산서(빌)에서 바로 차감 반영. 결제 흐름에 쿠폰 적용 로직.
- [ ] **8-8. 쿠폰 발송 추천 항목 + 탭 형식** — 쿠폰 보낼 때 상황별 추천(생일·재방문·휴면·리뷰감사 등)을 탭으로 골라 발송.
- [ ] **8-9. 조리 과정 업종별 맞춤** — 조리 단계 표현을 업종에 맞게(고기집=굽기, 카페=커피 추출 등). 기존 조리상태(KitchenDisplay/tableFlow)를 업종 프리셋으로.
- [ ] **8-10. 브랜드 설정 가독성/구조 개선** — BrandSettings에 결제·토스플레이스·AI예약·테마·마케팅·영업시간·POS 등 기능이 뒤죽박죽 섞여 어지러움. 카테고리별 섹션/탭 그룹화 + 시각적 위계 정리(접기/아코디언, 그룹 헤더, 우선순위 순서). "자주 쓰는 것 위 / 고급·연동 아래" 구조로.

> 진행 순서 제안: **쿠폰 묶음(8-6 버그→8-7→8-5→8-8)** → **8-9(조리 프리셋)** → 큰 기능(**8-2 사이트 / 8-3 세무 / 8-4 지원정보**). 8-1은 폰트 예시 대기.

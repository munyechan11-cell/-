...
- [x] **8-1. 폰트 여러 개 (사이트 글꼴 프리셋)** ✅ — 큐레이션 5종(에디토리얼/모던/클래식명조/둥근친근/시크세리프, Google Fonts 한글지원). BrandSettings 피커(실폰트 미리보기) + StoreSite 적용(CSS변수). 부제·주소도 함께(4836d61).
...
> ✅ **8-1~8-10 전부 완료·배포.** (8장 신규요청 100% 완료)

---

## 보안·정합성 후속 (적대적 검증 발견 — 큰 변경이라 별도)

> 검증 워크플로로 쿠폰/사이트 9건 점검. 작은 7건(할인 중복생성·리뷰쿠폰 누적·갤러리 동의·이미지 IDOR·알림 오탐·토스트·owner 가드)은 **즉시 수정·배포 완료**. 아래 2건만 구조적이라 별도.

- [ ] **쿠폰 자가발급 신뢰경계(#3, high)** — `coupons` 룰이 `signedIn()` 뿐이라 손님이 콘솔/스크립트로 임의 금액 'review'/'amount' 쿠폰을 자가발급 → 8-7 경로로 계산서 차감 가능(앱 전체가 client-trust 구조). 완화: `discount=min(amount, billTotal)` 캡으로 음수 환급은 불가(0원화까지만). 정식: 금액쿠폰 발급·승인을 Admin SDK 서버 엔드포인트로 이전 + Custom Token 인증 도입.
- [ ] **할인 라인 카드/현금 귀속(#1, high)** — 쿠폰주인≠결제자(공동 테이블, 타인 카드결제) 시 음수 할인 라인이 카드 결제 집합에 안 잡혀 cash로 휩쓸림 → 결제수단별 장부 왜곡 + 할인 미전달. **동일인 카드결제(일반 케이스)는 정상.** 정식: 결제 대상 산정을 customerId→tableNumber 기준으로 통일(결제 흐름 변경).
